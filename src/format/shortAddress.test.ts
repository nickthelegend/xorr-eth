 ...
 *
 * The contract these pin: absent formats as a dash, and short input is returned rather than mangled
 * into an ellipsis that identifies nothing.
 */
import { describe, expect, it } from 'vitest';
import { MINUS, shortAddress } from './index';
...
describe('shortAddress', () => {
  ...
  it('formats absence as a dash rather than throwing', () => {
    // The three ways a caller can have nothing, all from real API shapes.
    expect(shortAddress(undefined)).toBe(MINUS);
    expect(shortAddress(null)).toBe(MINUS);
    expect(shortAddress('')).toBe(MINUS);
  });
