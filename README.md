...
| **1inch — SwapVM** | `XorrSwapVMBook` compiles the terms of a trade into SwapVM program bytecode — a deadline, a slippage floor, a fee, a salt — so the *rules* of the fill are enforced inside the VM rather than trusted to whoever submits it. | **Wired.** 10 fork tests on the contract, including guards proving the deadline really expires and the fee really costs, plus `venues/swapvm.ts` in the settlement path: the executor discovers shipped programs from Aqua's own logs (filtered to the SwapVM router as the app), asks `delegatedFillArgs` to compute the call, and routes to it ahead of the aggregator. The activity log names the venue |
