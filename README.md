...
| **1inch — SwapVM** | `XorrSwapVMBook` compiles the terms of a trade into SwapVM program bytecode — a deadline, a slippage floor, a fee, a salt — so the *rules* of the fill are enforced inside the VM rather than trusted to whoever submits it. | **Contract only.** Written, deployed and covered by 10 fork tests, including behavioural guards proving the deadline really expires and the fee really costs — but **the running executor never calls it**. Aqua is the wired venue; this one is not. Said plainly because "Done" reads as "the app uses it", and it does not |
